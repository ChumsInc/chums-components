...
var DEFAULT_EVENTS$1 = ["mousedown", "touchstart"];
function useClickOutside(callback, events, nodes, enabled = true) {
	const ref = (0, import_react.useRef)(null);
	const eventsList = events || DEFAULT_EVENTS$1;
...
function usePagination({ total, siblings = 1, boundaries = 1, page, initialPage, onChange, startValue = 1 }) {
	...
	return {
		range: (0, import_react.useMemo)(() => {
			if (siblings * 2 + 3 + boundaries * 2 >= _total) return range$1(_startValue, _endValue);
			...
			if (!shouldShowLeftDots && shouldShowRightDots) return [
				...range$1(_startValue, _startValue + (siblings * 2 + boundaries + 2) - 1),
				DOTS,
				...range$1(_endValue - (boundaries - 1), _endValue)
			];
			if (shouldShowLeftDots && !shouldShowRightDots) {
				const rightItemCount = boundaries + 1 + 2 * siblings;
				return [
					...range$1(_startValue, _startValue + boundaries - 1),
					DOTS,
					...range$1(_endValue - rightItemCount, _endValue)
				];
			}
			return [
				...range$1(_startValue, _startValue + boundaries - 1),
				DOTS,
				...range$1(leftSiblingIndex, rightSiblingIndex),
				DOTS,
				...range$1(_endValue - boundaries + 1, _endValue)
			];
		}, [
...
function useSet(values) {
	const [set, setSet] = (0, import_react.useState)(() => new Set(values));
	const setRef = (0, import_react.useRef)(set);
	setRef.current = set;
	set.add = (...args) => {
...
var DEFAULT_EVENTS = ["mouse", "touch"];
...
function useLongPress(onLongPress, options = {}) {
	...
	const startPosition = (0, import_react.useRef)(null);
...
function useMask(options) {
	...
	const displayValueRef = (0, import_react.useRef)("");
	const rawValueRef = (0, import_react.useRef)("");
	...
	const redoStackRef = (0, import_react.useRef)([]);
	const getOptions = (0, import_react.useCallback)(() => {
		const opts = optionsRef.current;
		return getResolvedOptions(opts, rawValue);
	}, [rawValue]);
	const applyValue = (0, import_react.useCallback)(({ reprocessed, newRaw, displayValue, resolvedSlots, cursorPos, notifyChange }) => {
		const opts = optionsRef.current;
	...
	const initializeInputValue = (0, import_react.useCallback)((node) => {
		const opts = optionsRef.current;
	...
	const pushUndoState = (0, import_react.useCallback)(() => {
	...
	const applyHistoryState = (0, import_react.useCallback)((target) => {
		const opts = optionsRef.current;
...
//#endregion
//#region ../../node_modules/@mantine/hooks/esm/use-splitter/use-splitter.mjs
var PX_RE = /^(-?[\d.]+)px$/;
var REM_RE = /^(-?[\d.]+)rem$/;
var PERCENT_RE = /^(-?[\d.]+)%$/;
...
function applyAdjacentOnly(sizes, panels, handleIndex, delta) {
	...
	const newBefore = clamp$1(result[beforeIdx] + delta, effectiveBeforeMin, effectiveBeforeMax);
...
function useSplitter(options) {
	...
	const [activeHandle, setActiveHandle] = (0, import_react.useState)(-1);
	const [containerSize, setContainerSize] = (0, import_react.useState)(0);
	const optionsRef = (0, import_react.useRef)(options);
	optionsRef.current = options;
	const internalStateRef = (0, import_react.useRef)(createInitialInternalState());
	const containerRef = (0, import_react.useRef)(null);
	const containerSizeRef = (0, import_react.useRef)(0);
	const rootFontSizeRef = (0, import_react.useRef)(16);
	const documentControllerRef = (0, import_react.useRef)(null);
	const frameRef = (0, import_react.useRef)(0);
	const currentSizesRef = (0, import_react.useRef)(currentSizes);
	currentSizesRef.current = currentSizes;
	const preCollapseSizesRef = (0, import_react.useRef)(defaultSizes);
	const collapsed = currentSizes.map((size) => sizeMagnitude(size) === 0);
	const measureContainer = (0, import_react.useCallback)(() => {
		const node = containerRef.current;
		if (!node) return 0;
		const rect = node.getBoundingClientRect();
		return (optionsRef.current.orientation ?? "horizontal") === "horizontal" ? rect.width : rect.height;
	}, []);
	const updateSizes = (0, import_react.useCallback)((newSizes) => {
		currentSizesRef.current = newSizes;
		setCurrentSizes(newSizes);
	}, [setCurrentSizes]);
	const collapsePanel = (0, import_react.useCallback)((panelIndex) => {
	...
	const expandPanel = (0, import_react.useCallback)((panelIndex) => {
		if (!panels[panelIndex]?.collapsible) return;
		const raw = currentSizesRef.current;
		if (sizeMagnitude(raw[panelIndex]) !== 0) return;
		const container = pixelMode ? containerSizeRef.current || measureContainer() : 0;
	...
	const toggleCollapsePanel = (0, import_react.useCallback)((panelIndex) => {
	...
	const emitCollapseTransitions = (0, import_react.useCallback)((prev, next, indices, preCollapseSnapshot) => {
	...
	const reset = (0, import_react.useCallback)((handleIndex) => {
		const raw = currentSizesRef.current;
		const beforeIdx = handleIndex;
		const afterIdx = handleIndex + 1;
	...
	const containerRefCallback = (0, import_react.useCallback)((node) => {
		containerRef.current = node;
	}, []);
	(0, import_react.useEffect)(() => {
	...
	const handleRefCallbacks = (0, import_react.useRef)(/* @__PURE__ */ new Map());
	const handleElementControllers = (0, import_react.useRef)(/* @__PURE__ */ new Map());
	const getHandleRefCallback = (0, import_react.useCallback)((handleIndex) => {
	...
	const getHandleProps = (0, import_react.useCallback)((input) => {
		const { index } = input;
