...
const SortableTable = ({ fields, data, currentSort, onChangeSort, keyField, size = '', rowClassName, onSelectRow = noop, selected = '', className = '', tfoot, children, }) => {
    const tableClassName = classNames('table', className, {
        [`table-${size}`]: !!size,
    });
    return (_jsxs("table", { className: tableClassName, children: [_jsx(SortableTableHead, { currentSort: currentSort, fields: fields, onChangeSort: onChangeSort }), !!data.length && (_jsx("tbody", { children: data.map(row => {
